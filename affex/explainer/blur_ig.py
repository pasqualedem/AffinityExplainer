...
class FSSBlurIG(CoreSaliency):
    ...
    def GetMask(
        ...
        if sqrt:
            sigmas = [
                math.sqrt(float(i) * max_sigma / float(steps))
        ...
        for i in range(steps):
            x_step = np.stack(
                [gaussian_blur(x_value[k], sigmas[i]) for k in range(x_value.shape[0])]
            )
            gaussian_gradient = (
                np.stack(
                    [
                        gaussian_blur(x_value[k], sigmas[i] + grad_step)
                        for k in range(x_value.shape[0])
                    ]
                )
                - x_step
            ) / grad_step
            ...
            if len(x_step_batched) == batch_size or i == steps - 1:
                ...
                self.format_and_check_call_model_output(
                    call_model_output, x_step_batched.shape, self.expected_keys
                )

                tmp = step_vector_diff[i] * np.multiply(
                    gaussian_gradient_batched, call_model_output[INPUT_OUTPUT_GRADIENTS]
                )
