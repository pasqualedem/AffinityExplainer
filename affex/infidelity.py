...
def _make_next_infidelity_tensors_func(
    ...
    def _next_infidelity_tensors(
        ...
        # Normalize images (perturbations and attributions)  to [0, 1] range
        attributions_expanded = tuple(
            (attribution - attribution.min()) / (attribution.max() - attribution.min() + 1e-8)
            for attribution in attributions_expanded
        )
        perturbations_formatted = tuple(
            (perturbation - perturbation.min()) / (perturbation.max() - perturbation.min() + 1e-8)
            for perturbation in perturbations_formatted
        )
        ...
        attr_times_perturb_sums = sum(
            torch.mean(attribution_times_perturb, dim=1)
        ...
        perturbed_fwd_diffs = perturbed_fwd_diffs.view(bsz, -1).abs().mean(dim=1, keepdim=True)
...
def perturb_images(inputs, num_segments=80, compactness=10):
...
class FSSInfidelity(Metric):
    def __init__(self, model, n_perturb_samples=10, slic_segments=80, max_examples_per_batch=1):
        super().__init__()
        self.model = model
    ...
    def update(self, input_dict, explanation, explanation_mask, gt):
        ...
        infidelity_score = infidelity(
            ...
            perturb_func=lambda x: perturb_images(
                x, num_segments=self.slic_segments
            ),
            inputs=images,
            attributions=explanation.to(device),
