...
from .._vendor.dinov2.models import vision_transformer as vits
from .._vendor.dinov2.utils import utils as dinov2_utils
from .._vendor.dinov2.data.transforms import MaybeToTensor, make_normalize_transform

from .k_means import kmeans_pp
...
class Matcher:
    ...
    def patch_level_matching(self, ref_feats, tar_feat):
        ...
        # Affinity-noise intervention (appendix experiment): zero-mean noise scaled to the
        # similarity spread, applied on the decision path. No-op unless sim_noise_std is set.
        if getattr(self, "sim_noise_std", 0):
            S = S + torch.randn_like(S) * self.sim_noise_std * S.std()
        C = (1 - S) / 2  # distance
        ...
        # --- Expose the surviving support<->query prompt matches (Match->Seg interface) ---
        # indices_forward[0] indexes into S_forward (foreground support patches); map back to
        # the original support-patch index via indices_mask. These support patches are the ONLY
        # ones that produce SAM point prompts, so they are what a decoder-aware AffEx attributes.
        self._matched_sup_idx = indices_mask[indices_forward[0][sim_filter]]  # orig support-patch idx
        self._matched_qry_idx = points_matched_inds                          # query-patch idx (prompt)
        self._matched_sim = sim_matched[sim_filter]                          # matching score of each pair
