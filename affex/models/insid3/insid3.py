...
class INSID3(nn.Module):
    ...
    def _get_reference_prototype(self, ref_masks: torch.Tensor, feat_refs_deb: torch.Tensor, S: int, h: int, w: int):
    ...
    def predict(self, ref_images: torch.Tensor, ref_masks: torch.Tensor, tgt_image: torch.Tensor, return_intermediates: bool = False, embeddings: torch.Tensor = None) -> torch.Tensor | dict:
        ...
        Args:
            ...
            return_intermediates: bool to specify returning a dict of outputs
            embeddings: Optional (S+1, C', H', W') pre-extracted features for support and query images. If provided, these will be used instead of extracting features from the encoder.

        Returns:
            pred_mask OR dict containing pred_mask and intermediates.
        """
        ...
        for m in range(S):
            ...
            # Affinity-noise intervention (appendix experiment): no-op unless sim_noise_std is set.
            if getattr(self, "sim_noise_std", 0):
                sim_m = sim_m + torch.randn_like(sim_m) * self.sim_noise_std * sim_m.std()
            sim_maps.append(sim_m)
        # Dense support<->query correspondence per shot, stacked as (b, S, Hs, Ws, h, w).
        # This is the affinity tensor consumed by AffinityExplainer.
        sim_maps_stacked = torch.stack(sim_maps, dim=1)
        ...
        pred_mask_unrefined, prob_mask = self._seed_and_aggregate(
            candidate_mask, cluster_labels, cluster_protos, K,
            ref_prototype, feat_tgt, feat_tgt_deb, h, w
        )
        
        final_mask = self._finalize_mask(pred_mask_unrefined, tgt_image)
        if return_intermediates:
            return {
                "pred_mask": final_mask,
                "candidate_mask": candidate_mask.float().unsqueeze(0).unsqueeze(0),
    ...
    def _seed_and_aggregate(
        ...
        prob_mask = torch.zeros(h, w, dtype=torch.float32, device=cluster_labels.device)
        final_mask = torch.zeros(h, w, dtype=torch.bool, device=cluster_labels.device)
        
        valid = cluster_labels >= 0
        prob_mask[valid] = combined[cluster_labels[valid]]
