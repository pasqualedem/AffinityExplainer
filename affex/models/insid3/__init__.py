...
# Distillator (query-only student) is not needed for AffEx interpretability; it pulls
# extra deps (huggingface hub) and is omitted from this vendored copy.
...
def _build_encoder(model_size: str = "large"):
    # Offline clusters (e.g. Leo compute nodes have no internet): if DINOV3_HUB_DIR points at a
    # cached `facebookresearch_dinov3_main` checkout, load it via source="local" (no network).
    local_repo = os.environ.get("DINOV3_HUB_DIR")
    if local_repo and os.path.isdir(local_repo):
        return torch.hub.load(
            local_repo,
            _HUB_NAMES[model_size],
            source="local",
            weights=resolve(_WEIGHTS[model_size]),
        )
    return torch.hub.load(
...
class INSID3MultiClass(INSID3):
    ...
    def forward(self, x, postprocess=True):
        ...
        coarse_masks = []
        prob_masks = []
        # Per-class dense support<->query affinities for AffinityExplainer. Each entry is a
        # list of "levels" (INSID3 exposes a single DINOv3 level) whose tensor is laid out as
        # `b (hq wq) (n hs ws)`, matching the layout AffEx expects from DCAMA/DMTNet.
        attentions = []
        ...
        for c in range(masks.size(2)):
            ...
            # sim_maps: (b, n_shots, Hs, Ws, hq, wq) dense support<->query correspondence.
            # Reshape to AffEx's per-level attention layout `b (hq wq) (n hs ws)`.
            sim = predict_res["sim_maps"]
            sim = rearrange(sim, "b n hs ws hq wq -> b (hq wq) (n hs ws)")
            attentions.append([sim])
    ...
    def feature_ablation(self, result, chosen_class, mask, n_shots=None, explanation_size=None):
        """INSID3 exposes a single matching level, so per-level ablation weighting is moot.
        Return uniform per-level weights (equivalent to the `mean` aggregation the paper
        uses). Built on the logits' device to keep AffinityExplainer's aggregation on-GPU."""
        n_levels = len(result[ResultDict.ATTENTIONS][chosen_class])
        device = result[ResultDict.LOGITS].device
        return torch.full((n_levels,), 1.0 / n_levels, device=device)
